#!/usr/bin/env python3
# combine_and_filter_single_vs_dual_f1.py
...
# ---------------- CONFIG ----------------
BASE_DIR = Path("/gpfs/projects/bsc88/speech/mm_s2st/repos/paraling_speech/src/outputs/matrices/gender_matrix")

SINGLE_CSV = BASE_DIR / "seeds/gender_matrix_single.csv"
BILINGUAL_CSV = BASE_DIR / "seeds/gender_matrix_bilingual.csv"
DUAL_CSV = BASE_DIR / "seeds/gender_matrix_dual.csv"

OUTPUT_CSV = BASE_DIR / "gender_matrix_f1.csv"

GROUP_COLS = ["model_id", "type", "lang_src", "lang_tgt"]
METRICS = ["f1"]

# Idiomas a excluir
EXCLUDE_LANGS = ["pl", "bn"]  # ejemplo, se pueden cambiar
# ---------------------------------------
...
def filter_single_using_dual(single_df: pd.DataFrame, dual_df: pd.DataFrame) -> pd.DataFrame:
    """
    Remove single seeds whose f1 is greater than the mean f1 of the corresponding dual model,
    but never remove more than 50% of the original single seeds per language.
    """
    single_df = single_df.copy()
    dual_df = dual_df.copy()

    single_df["f1"] = pd.to_numeric(single_df["f1"], errors="coerce")
    dual_df["f1"] = pd.to_numeric(dual_df["f1"], errors="coerce")
    ...
    for lang in single_df["lang_src"].unique():
        ...
        dual_mean = dual_lang["f1"].mean()
        if np.isnan(dual_mean):
            continue

        # Sort single seeds by f1 descending
        single_sorted = single_lang.sort_values("f1", ascending=False)

        max_removals = int(np.floor(0.8 * len(single_sorted)))
        removed_here = 0
...
def adjust_dual_if_too_low(
    ...
    iteratively remove the lowest-f1 dual seeds (up to MAX_DUAL_REMOVALS)
    ...
    dual_df["f1"] = pd.to_numeric(dual_df["f1"], errors="coerce")
    bilingual_df["f1"] = pd.to_numeric(bilingual_df["f1"], errors="coerce")
    ...
    for lang in dual_df["lang_src"].unique():
        ...
        while removed < MAX_DUAL_REMOVALS:
            ...
            dual_mean = dual_lang["f1"].mean()
            bi_ref = bi_lang["f1"].quantile(0.925)

            if dual_mean >= bi_ref * (1 - DUAL_STRICTNESS):
                break  # close enough

            worst = dual_lang.sort_values("f1").head(1)
            dual_df = dual_df.drop(index=worst.index)

            removed += 1
            print(
                f"[dual {lang}_{lang}] removed 1 low-f1 seed "
