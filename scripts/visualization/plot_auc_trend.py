...
###############################################################
#   SIGMOID CRECIENTE PARA AUC
###############################################################
def sigmoid(x, a, b, c, d):
    """
    Función logística creciente:
    a  = valor asintótico mínimo (AUC más bajo)
    b  = amplitud (positiva)
    c  = pendiente
    d  = desplazamiento horizontal
    """
    return a + b / (1 + np.exp(-c * (x - d)))  # b > 0 produce curva creciente

###############################################################
#   LECTURA Y PROCESAMIENTO CSV
###############################################################
def process_csv(csv_path, max_samples=None, tolerance=0.06, blend=0.8, tolerance_up_multiplier=1.4):
    df = pd.read_csv(csv_path)
    if not {'auc', 'num_samples'}.issubset(df.columns):
        raise ValueError(f"CSV {csv_path} debe contener columnas: auc, num_samples")

    cols = ['num_samples', 'auc']
    ...
    # eliminar outliers extremadamente bajos (AUC debería subir)
    auc0 = df['auc'].iloc[0]
    df = df[df['auc'] >= auc0 / tolerance_up_multiplier]

    x = df['num_samples'].to_numpy()
    y = df['auc'].to_numpy()
    y_std = df['auc_std'].to_numpy() if 'auc_std' in df.columns else None

    # Ajuste inicial aproximado
    a0 = float(np.min(y))
    b0 = float(np.max(y) - np.min(y))  # positivo → curva creciente
    c0 = 0.0001
...
def compute_opt_interval(x_smooth, y_smooth, factor=2.0, num_candidates=200, min_samples=1):
    ...
    best_N = None
    best_avg = np.inf  # pendiente más positiva para AUC? → seguimos buscando mínimo de gradient negativo
    for N in candidates:
        x_left = N
        x_right = factor * N
...
###############################################################
#   PLOTS (idénticos pero cambiando títulos y labels a AUC)
###############################################################
def plot_individual(d, outdir, show_std=True, show_raw=True, outname=None):
    ...
    ax.set_title(f"AUC vs Number of Samples ({name})", fontsize=13)
...
def plot_combined(list_of_data, outdir, show_std=True, show_raw=True,
    ...
    ax.set_title("AUC vs Number of Samples (Multiple Experiments)", fontsize=14)
    ax.set_xlabel("Number of Samples")
    ax.set_ylabel("AUC")
    ax.grid(True, linestyle=':', linewidth=0.7, alpha=0.6)
...
def plot_sigmoid_only(list_of_data, outdir, outname="all_trend_only.png"):
    ...
    ax.set_title("AUC Trends (clean)", fontsize=14)
    ax.set_xlabel("Number of Samples")
    ax.set_ylabel("AUC")
    ax.grid(True, linestyle=':', linewidth=0.7, alpha=0.6)
    ax.legend(frameon=False, ncol=2)
...
def plot_logx(list_of_data, outdir, outname="all_trend_logx.png"):
    ...
    ax.set_xscale("log")
    ax.set_title("AUC Trends (log x-axis)", fontsize=14)
...
def main():
    parser = argparse.ArgumentParser(
        description="Plot AUC vs samples CSVs with logistic fit"
    )
    ...
    for csv_file in csv_files:
        ...
        path = plot_individual(d, args.outdir, show_std=show_std, show_raw=show_raw)
        print(f"✔ Saved individual: {path}")

    # Aplicar remove_outliers de forma robusta (isclose)
    for i, d in enumerate(list_of_data):
        if args.remove_outliers:
            # si x_plot está vacío, no conservar nada
            if d["x_plot"].size == 0:
                mask = np.zeros_like(d["x_raw"], dtype=bool)
            else:
                # para cada x_raw, verificar si coincide (con tolerancia) con algún x_plot
                mask = np.array([np.any(np.isclose(xi, d["x_plot"])) for xi in d["x_raw"]], dtype=bool)
    ...
    # Llamadas explícitas a las funciones con los parámetros que aceptan
    # plot_combined acepta show_std/show_raw -> pasarlos
    p1 = plot_combined(list_of_data, args.outdir, show_std=show_std, show_raw=show_raw,
                       outname="all_csvs_combined.png")
    print(f"✔ plot_combined: {p1}")

    # Estas funciones no esperan show_std/show_raw en su firma; llamarlas sin esos kwargs
    p2 = plot_sigmoid_only(list_of_data, args.outdir, outname="all_trend_only.png")
    print(f"✔ plot_sigmoid_only: {p2}")

    p3 = plot_logx(list_of_data, args.outdir, outname="all_trend_logx.png")
    print(f"✔ plot_logx: {p3}")

    p4 = plot_shaded_intervals(list_of_data, args.outdir, outname="all_trend_shaded_intervals.png", factor=2.0)
    print(f"✔ plot_shaded_intervals: {p4}")
...
python plot_auc_trend.py \
    ...
    --outdir /gpfs/projects/bsc88/speech/mm_s2st/repos/paraling_speech/scripts/visualization/plots/speaker/22.0/subsets_auc \
    --max-samples 120000 \
    --no-std \
    --remove-outliers 
"""