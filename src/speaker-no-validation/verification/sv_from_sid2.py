...
def main(argv=None):
    ...
    # leer fichero de pares con debug y fallback al primer idioma si hay "_"
    delim_pairs = "\t" if args.pairs_file.endswith(".tsv") or args.pairs_file.endswith(".tsv.gz") else ","
    pairs_path = os.path.expanduser(args.pairs_file)
    pairs_path = os.path.abspath(pairs_path)
    print(f"[DEBUG] pairs_path inicial: {pairs_path}, delimiter='{delim_pairs}'")

    if not os.path.exists(pairs_path):
        base = os.path.basename(pairs_path).split('.')[0]
        if "_" in base:
            lang_src = base.split("_")[0]

            orig_dir = os.path.dirname(pairs_path) or "."
            file_names = [
                f"{lang_src}.test.tsv",
                f"{lang_src}.test.tsv.gz",
                f"{lang_src}.tsv",
                f"{lang_src}.tsv.gz",
                f"{lang_src}.csv",
                f"{lang_src}.csv.gz",
            ...
            # posibles directorios donde buscar
            search_roots = [
                orig_dir,
                os.path.join(orig_dir, "04_test_pairs", "tsv"),
                os.path.join(os.path.dirname(orig_dir), "04_test_pairs", "tsv"),
                os.path.join(os.path.dirname(os.path.dirname(orig_dir)), "04_test_pairs", "tsv"),
            ]

            # deduplicar y convertir a absolute
            search_roots = [os.path.abspath(r) for r in dict.fromkeys(search_roots)]

            found = None
            candidates = []
            for r in search_roots:
                for fn in file_names:
                    cand = os.path.join(r, fn)
            ...
            if found:
                pairs_path = found
                print(f"[INFO] Usando test fallback: {pairs_path}")
            else:
                print("[WARN] No se encontró fallback. Comprobados los siguientes candidatos:")
                for c in candidates:
                    print("  ", c)
                # dejar que pd.read_csv genere el FileNotFoundError para debug
                print(f"[WARN] Intentando leer el fichero original: {pairs_path}")

    print(f"[DEBUG] Leyendo pairs desde: {pairs_path}")
    pairs_df = pd.read_csv(pairs_path, delimiter=delim_pairs)
    key_u, key_v, key_label = args.pairs_cols
    if key_u not in pairs_df.columns or key_v not in pairs_df.columns:
        raise RuntimeError(f"Pairs file {pairs_path} no contiene columnas {key_u}/{key_v}")
    ...
    # -------------------------
    # Guardar resumen en CSV en formato esperado por launcher
    # -------------------------
    try:
        # Nombre base del fichero de pares, por ej.:
        #   ab_ar.train.tsv  →  ab_ar
        #   en.train.tsv     →  en
        base = os.path.basename(args.pairs_file).split('.')[0]

        parts = base.split('_')
        if len(parts) == 2:
            # Caso bilingüe
            lang_src, lang_tgt = parts
        ...
        # Seed desde variable de entorno
        seed = os.environ.get("SEED", "NA")

        # CSV global
        SUMMARY_CSV = "/gpfs/projects/bsc88/speech/mm_s2st/repos/paraling_speech/src/outputs/speaker_matrix_single.csv"
        ...
        row = {
            ...
            "lang_src": lang_src,
            "lang_tgt": lang_tgt,
            "seed": int(seed) if seed != "NA" else seed,
            "eer": eer if eer is not None else "NaN",
            "auc": auc_val if auc_val is not None else "NaN",
            "threshold": thr if thr is not None else "NaN"
        }
        ...
        print(f"[SV] Appended summary to {SUMMARY_CSV}: {row}")
