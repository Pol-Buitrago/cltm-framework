...
# --- Paths and dataset
TASK="speaker"
SCRIPT_PATH="/gpfs/projects/bsc88/speech/mm_s2st/repos/paraling_speech/src/HuBERT/speaker-no-validation/identification/main.py"   # path to python training script (relative o absoluto)
...
DATA_DIR="${DATA_DIR:-${DATA_ROOT}${TASK}_verif/03_combined_train/tsv}"
...
# ---------------------------
if [ -z "${TIMESTAMP:-}" ]; then
  echo "ERROR: TIMESTAMP must be provided by the caller."
  exit 1
fi
export TIMESTAMP
...
case "${RUN_MODE}" in
  grid)
    ...
    srun /gpfs/projects/bsc88/speech/mm_s2st/repos/paraling_speech/src/HuBERT/speaker-no-validation/identification/utils/run_grid.sh
    ;;
  single)
    echo "Launching SINGLE worker via srun: utils/run_single.sh"
    srun /gpfs/projects/bsc88/speech/mm_s2st/repos/paraling_speech/src/HuBERT/speaker-no-validation/identification/utils/run_single.sh
    ;;
esac
