...
def main(argv=None):
    ...
    p.add_argument("--no_summary", action="store_true", help="No escribir CSV de resumen (útil en validación por checkpoints).")
    p.add_argument("--summary_id", type=str, default=None, help="Identificador opcional para incluir en el nombre del CSV de resumen (ej: 'val_ckpt-264' o 'final').")
    ...
    try:
        if getattr(args, "no_summary", False):
            print("[SV] no_summary requested, skipping summary CSV write.")
        else:
            ...
            num_samples_str = str(num_samples) if num_samples is not None else "NaN"

            out_dir_base = "/gpfs/projects/bsc88/speech/mm_s2st/repos/paraling_speech/src/outputs"
            os.makedirs(out_dir_base, exist_ok=True)

            # if summary_id provided, include in filename to avoid overwriting/appending ambiguities
            if getattr(args, "summary_id", None):
                sid = re.sub(r'\s+', '_', str(args.summary_id))
                csv_path = os.path.join(out_dir_base, f"{lang}.eer_by_samples_{seed}_{sid}.csv")
            else:
                csv_path = os.path.join(out_dir_base, f"{lang}.eer_by_samples_{seed}.csv")

            header = "eer,auc,thr,num_samples\n"
            row = f"{eer_str},{auc_str},{thr_str},{num_samples_str}\n"
