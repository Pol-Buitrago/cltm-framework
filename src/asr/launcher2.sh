...
# DRY_RUN can be set as env var, or passed as first argument --dry-run
...
# Logs directory (se asume que existe o se creará por los jobs)
LOGS_DIR="${RESULTS_DIR_BASE}/logs"
...
for lang in "${LANGS[@]}"; do
  ...
  for entry in "${sorted_entries[@]}"; do
    ...
    for seed in "${SEEDS[@]}"; do
      ...
      # -----------------------
      # Verificaciones para evitar reenvíos duplicados
      # 1) Resultados ya presentes en OUTFILE (tu chequeo original)
      # 2) Existe carpeta/archivo en logs con JOB_NAME (ya lanzado)
      # 3) Job ya en squeue (queued/running)
      # -----------------------

      OUTFILE="${RESULTS_DIR_BASE}/${lang}.wer_by_samples_${seed}.csv"
      SKIP_SUBMIT=false

      # 1) Comprobar CSV de resultados existente (tu lógica actual)
      ...
      # 2) Comprobar en logs (por prefijo JOB_NAME). Usa compgen para evitar globbing fallido
      if [ "${SKIP_SUBMIT}" = false ]; then
        if [ -d "${LOGS_DIR}" ]; then
          if compgen -G "${LOGS_DIR}/${JOB_NAME}*" > /dev/null; then
            printf 'Already launched (logs exist): %s -> %s\n' "${JOB_NAME}" "${LOGS_DIR}"
            SKIP_SUBMIT=true
          fi
        fi
      fi

      # 3) Comprobar si ya está en squeue (queued/running) — opcional si quieres máxima seguridad
      if [ "${SKIP_SUBMIT}" = false ]; then
        if command -v squeue >/dev/null 2>&1; then
          # buscar nombre exacto en la columna de nombre de job
          if squeue -h -o "%j" | grep -Fxq "${JOB_NAME}"; then
            printf 'Already in squeue: %s\n' "${JOB_NAME}"
            SKIP_SUBMIT=true
          fi
        fi
      fi

      # Si quedó marcado para saltar, continuamos al siguiente
      ...
      if [ "${DRY_RUN}" = "true" ]; then
        # En dry-run imprimimos exactamente el comando sbatch que se ejecutaría
