#!/usr/bin/env python3
"""
Adaptación del script original (SpeechLLM-based) para soportar ejecución
"por subsets" exactamente con la misma interfaz y comportamiento que
sv_ecapa_batch_subsets_seeded.py.

Modo de uso:
 - Modo normal (igual que el script original): pasar --test_tsv (y opcionalmente --train_tsv, --llm_model, etc.)
 - Modo batch por subsets: pasar --subsets_root and --out_root (seguirá la lógica por-lenguaje, por-subset, por-seed).

CSV result files (por idioma y seed) tendrán columnas:
    eer,auc,thr,num_samples

Si deseas solo la versión ECAPA-style que compartiste antes, este script NO cambia la pipeline: cuando se usa en modo batch
llamará internamente a la función train_and_eval (que mantiene la funcionalidad original basada en SpeechLLM).
"""
import os
import argparse
import random
...
def train_and_eval(args):
    """
    Versión adaptada del train_and_eval original, usando SpeechLLM para extraer embeddings.
    Se mantiene compatibilidad con la interfaz del runner por subsets: devuelve dict con 'eer','auc','thr'.
    """
    set_seed(args.seed)
    ...
    train_dataset = None
    if getattr(args, "train_tsv", None):
        train_dataset = SingleUtteranceDataset(args.train_tsv, max_seconds=getattr(args, "max_seconds", 3.0),
    ...
    if train_dataset is not None:
        ...
        train_emb_cache = compute_embeddings_llm(train_paths_unique, embedder, device,
                                                 sample_rate=getattr(args, "sample_rate", 16000),
                                                 max_seconds=getattr(args, "max_seconds", 3.0),
                                                 batch_size=getattr(args, "batch_size", 16))

        # Construir pares de entrenamiento
        print(f"Generating up to {getattr(args, 'pairs_per_epoch', 20000)} training pairs...")
        train_pairs = build_train_pair_dataset(train_dataset, train_emb_cache, pairs_per_epoch=getattr(args, 'pairs_per_epoch', 20000), rng_seed=args.seed)
        ...
        if len(train_pairs) > 0:
            ...
            opt = optim.Adam(clf.parameters(), lr=getattr(args, "lr", 1e-3))
            criterion = nn.BCEWithLogitsLoss()

            # training loop
            bsz = 256
            epochs = max(1, getattr(args, "epochs", 1))
    ...
    # ------------------------
    if not getattr(args, "test_tsv", None):
    ...
    emb_cache = compute_embeddings_llm(unique_paths, embedder, device, sample_rate=getattr(args, "sample_rate", 16000),
                                      max_seconds=getattr(args, "max_seconds", 3.0), batch_size=getattr(args, "batch_size", 16))
...
# ------------------------
# Batch orchestration (mismo comportamiento que sv_ecapa_batch_subsets_seeded.py)
# ------------------------
...
def count_train_samples(train_tsv: Path):
    ...
    with open(train_tsv, 'r', encoding='utf-8') as f:
        for i, line in enumerate(f):
            if i == 0:
                # skip header-ish line like in ECAPA runner
                continue
            if line.strip():
                c += 1
    return c
...
# ------------------------
# Unified CLI (combina opciones del script original y del batch-runner)
# ------------------------
def parse_args():
    p = argparse.ArgumentParser()
    # batch options (if provided, script runs in batch mode)
    p.add_argument("--subsets_root", required=False, help="Root path containing language subdirs with subsets (batch mode)")
    p.add_argument("--out_root", required=False, help="Directory to store per-language, per-seed CSV results (batch mode)")
    p.add_argument("--langs", default="all", help="Comma-separated language codes to process (default all) (batch mode)")
    p.add_argument("--seeds", default="42", help="Comma-separated seeds to run (e.g. 42,2025) (batch mode)")
    p.add_argument("--dry_run", action='store_true', help="When batch mode: do not actually train, only show planned runs")

    # original script options (single-run mode)
    ...
    p.add_argument("--llm_model", type=str, default=None, help="Local path to SpeechLLM repo (optional). If omitted, script will try default local path.")
...
# ------------------------
# Batch main: if --subsets_root passed, run in batch mode similar to ECAPA runner
# ------------------------
def main_batch(args):
    ...
    for lang_dir in sorted(lang_dirs):
        ...
        for seed in seeds:
            ...
            for subset_dir in subset_dirs:
                ...
                # prepare per-run args for train_and_eval (map to fields expected by train_and_eval)
                run_outdir = out_root / f"runs/run_{lang}_{subset_dir.name}_seed{seed}"
                run_args = argparse.Namespace(
                    train_tsv=str(train_tsv),
                    test_tsv=str(test_tsv),
                    out_dir=str(run_outdir),
                    epochs=args.epochs,
                    ...
                    local_speechllm_dir=getattr(args, "local_speechllm_dir", SPEECHLLM_LOCAL_DIR),
                    text_tokenizer=getattr(args, "text_tokenizer", None),
                    pre_instruction=getattr(args, "pre_instruction", None),
                    post_instruction=getattr(args, "post_instruction", None),
                    out_prefix=getattr(args, "out_prefix", None),
    ...
    print("All done (batch).")


# ------------------------
# Normal single-run main (compatibilidad con script original)
# ------------------------
def main_single(args):
    # prepare args for train_and_eval directly
...
def main():
    args = parse_args()
    # If subsets_root and out_root provided -> batch mode
    if getattr(args, "subsets_root", None) and getattr(args, "out_root", None):
        main_batch(args)
    else:
        # run single mode (behaviour like original script)
        main_single(args)
